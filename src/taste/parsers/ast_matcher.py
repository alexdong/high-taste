...
def check_try_except_defensive(node: ast.Try, source_lines: list[str]) -> CodeViolation | None:
...
def check_repeated_file_loads(node: ast.Call, source_lines: list[str]) -> CodeViolation | None:
...
def check_deep_nesting(node: ast.If, source_lines: list[str]) -> CodeViolation | None:
...
def check_hardcoded_lists(node: ast.List, source_lines: list[str]) -> CodeViolation | None:
    """Check for hardcoded lists that could be dynamic (Rule 004)."""
    # Look for lists with string literals that might be file names
    if len(node.elts) >= 3:  # Only flag lists with multiple items
        string_count = 0
        ...
        if string_count >= 3 and has_file_extension:
            return CodeViolation(
                rule_id="004",
                line_number=node.lineno,
                column=node.col_offset,
...
def check_tuple_returns(node: ast.Return, source_lines: list[str]) -> CodeViolation | None:
    """Check for tuple returns that should use NamedTuple (Rule 005)."""
    if isinstance(node.value, ast.Tuple | ast.List) and len(node.value.elts) >= 2:
        ...
        if len(types_seen) >= 2:  # Mixed types suggest structured return
            return CodeViolation(
                rule_id="005",
                line_number=node.lineno,
                column=node.col_offset,
...
def check_database_commit_order(node: ast.Call, source_lines: list[str]) -> CodeViolation | None:
...
def analyze_python_code(source_code: str, patterns: list[ASTPattern] | None = None) -> list[CodeViolation]:
